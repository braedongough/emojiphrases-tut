    ...
    post<Signup> {
        ...
        when {
            password.length < MIN_PASSWORD_LENGTH ->
                call.redirect(signUpError.copy(error = "Password must be 6 long"))
            userId.length < MIN_USER_ID_LENGTH ->
                call.redirect(signUpError.copy(error = "Username should be more then 3 long"))
            db.user(userId) != null ->
                call.redirect(signUpError.copy(error = "Username already taken"))
            else -> {
                ...
                try {
                    ...
                    when {
                        ...
                        db.userByEmail(email) != null ->
                            call.redirect("Email already registered")
                        else -> {
